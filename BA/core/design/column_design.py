...
def design_rectangular_column(Nu, Mu, height, k_factor, section, beton, acier):
    ...
    if elancement_lambda > 50:
        ...
        v_print("Statut poteau", "λ <= 50", f"{elancement_lambda:.2f} <= 50", "Non-élancé")
    ...
    v_print("Section d'acier théorique", "As = Ma / (z*fsu)", f"{Ma_Nm/1e6:.4f} / ({z:.3f} * {f_su:.2f})", A_requise_cm2, "cm²")
...
def design_column_compression_bael(Nu, height, k_factor, section, beton, acier):
    ...
    if elancement_lambda <= 50:
        ...
        v_print("Coefficient alpha (λ<=50)", "α = 0.85/(1+0.2(λ/35)²)", f"f(λ={elancement_lambda:.2f})", alpha)
    elif 50 < elancement_lambda <= 70:
        alpha = 0.60 * (50 / elancement_lambda)**2
        v_print("Coefficient alpha (λ>50)", "α = 0.60 * (50/λ)²", f"f(λ={elancement_lambda:.2f})", alpha)
    else:
        return {"status": "ERREUR", "message": f"Élancement λ={elancement_lambda:.1f} > 70. Méthode non applicable."}
    ...
    if (Nu / alpha) > beton_res_MN:
        ...
        A_requise_cm2 = A_requise_m2 * 10000
    v_print("Section d'acier (calcul)", "As = [(Nu/α)-Res_beton]*γs/fe", "...", A_requise_cm2, "cm²")
