...
def check_concrete_compression_stress(Nser_kN, section, beton):
    ...
    v_print(
        "Contrainte béton (ELS)",
        "σ_bc = Nser / B",
        f"{Nser_MN:.3f} / {area_m2:.3f}",
        sigma_bc_acting,
        "MPa",
    )
    v_print(
        "Contrainte limite béton",
        "σ_lim = 0.6 * fc28",
        f"0.6 * {beton.fc28}",
        sigma_bc_limit,
        "MPa",
    ...
    if sigma_bc_acting <= sigma_bc_limit:
        return {
            "status": "OK",
            "message": "Contrainte de compression du béton vérifiée.",
        }
    else:
        return {
            "status": "NON CONFORME",
            "message": "Contrainte de compression du béton dépassée. Augmenter la section.",
        }
...
def check_soil_bearing_pressure(total_p_ser_kN, radier_dims):
    ...
    v_print(
        "Contrainte au sol (ELS)",
        "σ_ser = ΣP_ser / S",
        f"{total_p_ser_kN:.1f} / {surface:.2f}",
        sigma_sol_ser,
        "kPa",
    )
