...
def cmd_optimize(
    ...
    method: Optional[str] = typer.Option(None, "--method", help="Override de la méthode: binary|nested|global"),
    ...
    if method not in ("binary", "nested", "global"):
        typer.secho("Méthode non supportée pour le Jalon 2 (support: binary, nested, global).", fg=typer.colors.RED)
    ...
    if method == "binary":
        ...
        cfg_obj = ConfigurationOptimisation(**(cfg_raw.get("optimization", {}))) if hasattr(ConfigurationOptimisation, "__init__") else cfg_raw.get("optimization", {})
    ...
    costs = scorer.compute_total_cost(nm.__dict__ if hasattr(nm, "__dict__") else nm, diameters_current, res.get("H_tank_m", H_max)) if res.get("feasible") else {"CAPEX": None, "OPEX_annual": None, "total_cost": None}

    # Enrichir payload rapport
    report_payload = {
    ...
    res["costs"] = costs
    res["report_payload"] = report_payload
    res.setdefault("meta", {})
