...
def prevoir_population(donnees: dict) -> dict:
    """
    Estime la population future selon plusieurs méthodes :
    - arithmetique (ou lineaire)
    - geometrique (ou exponentielle, malthus)
    - logistique
    - malthus (alias geometrique)
    - exponentiel (alias geometrique)
    Les formules sont strictement celles du cahier des charges.
    """
    methode = donnees.get("methode")
    t_futur = donnees.get("annee_projet")
    resultats = {"methode": methode, "annee_projet": t_futur}
    try:
        if methode in ["arithmetique", "lineaire"]:
            y1, t1 = donnees["pop_annee_1"]
            y2, t2 = donnees["pop_annee_2"]
            ku = (y2 - y1) / (t2 - t1)
            pop_future = y2 + ku * (t_futur - t2)
            resultats["population_estimee"] = int(pop_future)
            resultats["formule"] = "y = y2 + Ku * (t - t2)"
            resultats["ku"] = ku
            ...
            pop_future = math.exp(log_pop_future)
            resultats["population_estimee"] = int(pop_future)
            resultats["formule"] = "y = exp(ln(y2) + Kp * (t - t2))"
            resultats["kp"] = kp
            ...
            resultats["formule"] = "Yc = K / (1 + 10^(a - b*x))"
            resultats["a"] = a
            resultats["b"] = b
        else:
