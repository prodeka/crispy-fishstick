...
def check_and_install_packages(packages):
    ...
    missing_packages = [
        pkg for pkg in packages if importlib.util.find_spec(pkg.split("==")[0]) is None
    ]
    ...
    if missing_packages:
        ...
        for package in missing_packages:
            try:
                ...
                print_colored(
                    f"Veuillez l'installer manuellement : pip install {package}", "red"
                )
                sys.exit(1)
...
def get_input_with_default(prompt, default, type_converter=float):
    ...
    while True:
        user_input = (
            input(f"{prompt} (ou '<' pour retour) [défaut: {default}] : ")
            .strip()
            .replace(",", ".")
        )
        if user_input == "<":
        ...
        try:
            ...
            print_colored("Entrée invalide. Veuillez entrer un nombre ou '<'.", "red")


def get_menu_choice(prompt, options, extra_options=None):
    """
    Affiche un menu numéroté et retourne la clé du choix de l'utilisateur.
    Gère une option de retour standard '<' et des options supplémentaires personnalisées.

    Args:
        prompt (str): L'invite affichée à l'utilisateur.
        options (dict): Le dictionnaire des choix principaux.
        extra_options (dict, optional): Un dictionnaire d'options spéciales comme {'<<': 'Changer de fichier'}.
    """
    ...
    while True:
        ...
        for key, value in extra_options.items():
            print_colored(f"  [{key}] {value}", "yellow")

        # Toujours proposer une option de retour standard
        if "<" not in extra_options:
            print_colored("  [<] Retour", "yellow")

        choix = input("Votre choix : ").strip().lower()

        if choix == "<":
            return "<"  # Retour standard
        if choix in options or choix in extra_options:
            return choix
