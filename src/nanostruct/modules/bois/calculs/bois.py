...
def verifier_section_bois(
    ...
    if verbose:
        print(
            f"Propriétés: {classe_bois} ({type_bois}), fm,k={fm_k} MPa, E0,mean={E0_mean_MPa} MPa"
        )
    ...
    I_mm4 = (b * h**3) / 12
    fleche_inst_mm = (
        (5 * charge_ser_N_mm * (longueur * 1000) ** 4) / (384 * E0_mean_MPa * I_mm4)
        if I_mm4 > 0
        else float("inf")
    )
    ...
    if verbose:
        ...
        print(
            f"   Remplacement: f_md = ({k_mod} * {fm_k}) / {gamma_M} = {f_md:.2f} MPa"
        )
        print("\n   Formule (Contrainte): sigma_m_d = M_Ed / W")
        print(
            f"   Remplacement: sigma_m_d = {M_Ed_Nmm:.0f} / {W_mm3:.0f} = {sigma_m_d:.2f} MPa"
        )
        print(
            f"   Condition: {sigma_m_d:.2f} MPa <= {f_md:.2f} MPa ? -> {'OK' if resistance_ok else 'INSUFFISANTE'}"
        )

        print("\n2. Vérification de la flèche (ELS):")
        print("   Formule (Flèche finale): f_fin = f_inst * (1 + k_def)")
        print(
            f"   Calcul: f_fin = {fleche_inst_mm:.2f} * (1 + {k_def}) = {fleche_fin_mm:.2f} mm"
        )
        print("\n   Formule (Flèche admissible): f_adm = L / 300")
        print(
            f"   Calcul: f_adm = {longueur * 1000:.0f} / 300 = {fleche_admissible_mm:.2f} mm"
        )
        print(
            f"   Condition: {fleche_fin_mm:.2f} mm <= {fleche_admissible_mm:.2f} mm ? -> {'OK' if fleche_ok else 'INACCEPTABLE'}"
        )

    est_valide = resistance_ok and fleche_ok
    message = (
        "La section est conforme." if est_valide else "La section n'est pas conforme."
    )
...
def verifier_traction_bois(
    b, h, effort_N_daN, classe_bois, classe_service, duree_charge, verbose=True
):
    """
    ...
    sigma_t0_d = (
        effort_N_Newton / aire_section_mm2 if aire_section_mm2 > 0 else float("inf")
    )
    ...
    if verbose:
        ...
        print(
            f"   Remplacement: f_t,0,d = ({k_mod} * {ft_0_k}) / {gamma_M} = {f_t0_d:.2f} MPa"
        )
        print("\n   Formule (Contrainte): sigma_t,0,d = N_Ed / A")
        print(
            f"   Remplacement: sigma_t,0,d = {effort_N_Newton:.0f} / {aire_section_mm2:.0f} = {sigma_t0_d:.2f} MPa"
        )
        print(
            f"   Condition: {sigma_t0_d:.2f} MPa <= {f_t0_d:.2f} MPa ? -> {'OK' if resistance_ok else 'INSUFFISANTE'}"
        )

    message = (
        "La section est conforme en traction."
        if resistance_ok
        else "La section n'est pas conforme en traction."
    )
