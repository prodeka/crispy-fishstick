...
def saisir_charges_flexion(verbose_mode):
    ...
    charge_g_repartie = click.prompt(
        "  -> Charge permanente G (plancher, etc.)", type=float, default=0.0
    )
    charge_q_repartie = click.prompt(
        "  -> Charge d'exploitation Q", type=float, default=0.0
    )
    ...
    charge_g_ponctuelle = click.prompt(
        "  -> Charge permanente G (cloison, poteau...)", type=float, default=0.0
    )
    charge_q_ponctuelle = click.prompt(
        "  -> Charge d'exploitation Q", type=float, default=0.0
    )
    ...
    if verbose_mode and (g_equiv_ponctuelle > 0 or q_equiv_ponctuelle > 0):
        click.echo(
            click.style(
                "\nNOTE: Charges ponctuelles converties en charges reparties equivalentes:",
                fg="blue",
            )
        )
...
def run_acier_mode(verbose_mode):
    ...
    nuance = click.prompt(
        "  -> Nuance de l'acier", default="S235", show_default=verbose_mode
    )
    fy = click.prompt(
        f"  -> Limite d'elasticite (fy) de '{nuance}' en MPa",
        default=235,
        type=float,
        show_default=verbose_mode,
    )
    E_module = click.prompt(
    ...
    sollicitations = calculer_sollicitations_completes(
        longueur, charges_entrees, "acier", "A", verbose=verbose_mode
    )
    ...
    click.echo(
        click.style(f"Le profile IPE recommande est : {profil_recommande}", fg="green")
    )
    click.echo("----------------------------------------------------")
...
def run_bois_flexion(verbose_mode):
    ...
    classe_bois = click.prompt(
        "  -> Classe du bois (ex: C24, D30, GL24h)", default="C24"
    )
    classe_service = click.prompt(
        "  -> Classe de service",
        type=click.Choice(["classe_1", "classe_2", "classe_3"]),
        default="classe_1",
    )
    duree_charge = click.prompt(
        "  -> Duree de la charge",
        type=click.Choice(["permanente", "long_terme", "moyen_terme", "court_terme"]),
        default="permanente",
    )
    if verbose_mode:
        click.echo("\n... Lancement du premier calcul de sollicitations ...")
    sollicitations = calculer_sollicitations_completes(
        longueur, charges_entrees, "bois", "A", verbose=verbose_mode
    )
    while True:
        click.echo(
            click.style(
        ...
        if est_valide:
            click.echo(
                click.style(
                    f"✅ RESULTAT: {message} La section {b}x{h} est ADEQUATE.",
                    fg="green",
                    bold=True,
                )
            )
            ...
            click.echo(
                click.style(
                    f"❌ RESULTAT: {message} La section {b}x{h} n'est PAS adequate.",
                    fg="red",
                    bold=True,
                )
            )
...
def run_batch_mode(filepath, verbose_mode):
    ...
    click.echo(
        click.style(
            f"\nLancement du Traitement par Lots sur le fichier: {filepath}",
            fg="cyan",
            bold=True,
        )
    )
    try:
        ...
        click.echo(
            click.style(
                f"Erreur: Le fichier '{filepath}' n'a pas ete trouve.", fg="red"
            )
        )
        return
    liste_resultats = []
    for index, poutre in df_entree.iterrows():
        ...
        charges = {
            "G": poutre.get("charge_G", 0),
            "Q": poutre.get("charge_Q", 0),
            "W": poutre.get("charge_W", 0),
            "S": poutre.get("charge_S", 0),
        }
        sollicitations = calculer_sollicitations_completes(
        ...
        profil_recommande = trouver_profil_acier(
            ...
            poutre["longueur"],
            sollicitations["p_ser"],
            nuance=poutre["nuance_acier"],
            fy_MPa=poutre["fy_MPa"],
            E_MPa=poutre["E_MPa"],
    ...
    click.echo(
        click.style(
            f"\nLes resultats ont ete enregistres dans le fichier: {fichier_sortie}",
            fg="yellow",
        )
    )
...
@click.option(
    ...
    "-f",
    default=None,
    help="Chemin vers le fichier CSV pour le traitement par lots.",
)
@click.option(
    "--silencieux", is_flag=True, help="N'affiche pas les etapes de calcul detaillees."
)
def main(fichier, silencieux):
    ...
    if fichier:
        ...
        click.echo(
            click.style(
                "\nBienvenue dans PyStruct, l'assistant de dimensionnement.",
                fg="blue",
                bold=True,
            )
        )
