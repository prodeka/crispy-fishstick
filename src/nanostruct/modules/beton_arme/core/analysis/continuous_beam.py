...
def analyze_by_forfaitaire(spans, load_wu):
    ...
    v_print("Moments sur appuis (kNm)", "[M_A, M_B, ...]", ", ".join([f"{m:.2f}" for m in moments_appui]), f"Min: {min(moments_appui):.2f}")
    v_print("Moments en travée (kNm)", "[M_AB, M_BC, ...]", ", ".join([f"{m:.2f}" for m in moments_travee]), f"Max: {max(moments_travee):.2f}")
