...
def design_raft_by_strip_method(poteaux_df, A, B, h, sigma_sol_adm, beton, acier):
    ...
    poteaux_df["P_ui"] = (
        1.35 * poteaux_df["Charge_G_MN"] + 1.5 * poteaux_df["Charge_Q_MN"]
    )
    ...
    if q_sol_ser > sigma_sol_adm:
        return {
            "status": "ERREUR",
            "message": f"Contrainte au sol ({q_sol_ser:.3f} MPa) dépasse l'admissible ({sigma_sol_adm:.3f} MPa).",
        }
    ...
    q_radier_ultime_par_m = (
        poteaux_df["P_ui"].sum() / (A * B)
    ) * 1.0  # Pression * 1m de large
