...
def _draw_callout(ax, target_pos, label_pos, label_text):
    ...
    ax.plot([target_pos[0], label_pos[0]], [target_pos[1], label_pos[1]], 'k-', lw=0.8, zorder=40)
    circle = patches.Circle(label_pos, radius=2.5, facecolor='white', edgecolor='black', lw=1, zorder=50)
    ax.add_patch(circle)
    ax.text(label_pos[0], label_pos[1], label_text, ha='center', va='center', fontsize=8, weight='bold', zorder=51)
    ...
    for group in bar_groups:
        ...
        for _ in range(group['qty']):
            if pos_idx < len(all_positions):
                ...
                ax.add_patch(patches.Circle((x, y), radius=diam_cm/2, fc='black', zorder=3))
                pos_idx += 1
...
def _draw_elevation_view(ax, results):
    ...
    ax.add_patch(patches.Rectangle((0, 0), b_poteau, h_poteau, lw=1.5, ec='black', fc='none'))
    ax.vlines(x=[enrobage, b_poteau - enrobage], ymin=enrobage, ymax=h_poteau-enrobage, color='black', lw=0.8, ls='--')
    y_stirrups = np.linspace(enrobage, h_poteau - enrobage, num_cadres)
    ax.hlines(y=y_stirrups, xmin=enrobage, xmax=b_poteau - enrobage, color='black', lw=0.8)
...
def _draw_nomenclature_table(ax, results):
    ...
    for group in results.get('bar_groups', []):
        cell_text.append([str(group['rep']), f"{group['qty']}xΦ{group['diam']}", '00', 'Droite'])
...
def plot_column_section(results, output_folder="output", filename="plan_ferraillage.png"):
