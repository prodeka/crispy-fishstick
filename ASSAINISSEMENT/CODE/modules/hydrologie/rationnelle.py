...
from modules.hydrologie.idf_formulas import calculer_intensite_pluie
...
def run_calcul_rationnelle(troncon, params_pluie: dict, tc_formule_name: str, verbose=False):
    ...
    for it_num in range(MAX_ITERATIONS):
        if verbose: 
            print_colored(f"\n--- Itération n°{it_num + 1} (tc = {tc_iteration:.2f} min) ---", "bold")
        
        # Calcul de l'intensité et du débit
        intensite = calculer_intensite_pluie(tc_iteration, params_pluie, verbose)
        troncon.q_max_m3s = calculer_q_max_rationnelle(troncon.c_moyen_cumule, intensite, troncon.surface_cumulee, verbose)
        
        # Tentative de dimensionnement
        try:
            resultat_dim_iter = dimensionner_section(troncon, troncon.q_max_m3s, verbose)
        ...
        temps_parcours = (troncon.long_troncon_m / vitesse) / 60 if vitesse > 0 else float('inf')
        tc_calcule_nouveau = tc_amont + temps_parcours
        
        # 4. Vérification de la convergence
        if not math.isinf(tc_calcule_nouveau) and abs(tc_calcule_nouveau - tc_iteration) < TOLERANCE_TC_min:
    ...
    if verbose: 
        print_colored("\n>>> ATTENTION: Convergence non atteinte après le nombre maximal d'itérations.", "red")