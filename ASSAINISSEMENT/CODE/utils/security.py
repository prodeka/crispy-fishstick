    ...
    while True: # Boucle principale pour permettre de réessayer
        ...
        for attempt in range(max_attempts):
            ...
            if remaining_attempts > 0:
                print_colored(f"Mot de passe incorrect. Il vous reste {remaining_attempts} essai(s).", "red")
            else:
        ...
        while True:
            retry = input("Voulez-vous réessayer l'authentification ? [o/n] : ").lower().strip()
            if retry == 'o':
                break # On sort de cette petite boucle pour recommencer la grande boucle
            elif retry == 'n':
                print_colored("Authentification échouée. Le programme va se fermer.", "red")
                return False # L'utilisateur abandonne
            else:
                print_colored("Réponse invalide. Veuillez entrer 'o' pour oui ou 'n' pour non.", "yellow")
...
def display_disclaimer_and_get_agreement():
    ...
    while True:
        ...
        if choice == 'o':
            ...
            print_colored("Vous avez refusé les conditions. Le programme va se fermer.", "red")
            return False
        else:
