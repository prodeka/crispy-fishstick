...
def calculer_sollicitations_completes(
    ...
    'charges_list' est une liste de dictionnaires, ex: [{'type': 'repartie', 'valeur': 5.0, 'categorie': 'G'}]
    """
    if verbose:
        print("\n--- Debut du calcul des sollicitations completes ---")
        print(f"Charges d'entree: {charges_list}")

    # Agrégation des charges par catégorie
    charges_agg = {'G': 0, 'Q': 0, 'W': 0, 'S': 0}
    for charge in charges_list:
        categorie = charge.get('categorie')
        valeur = charge.get('valeur', 0)
        if categorie in charges_agg:
            charges_agg[categorie] += valeur
