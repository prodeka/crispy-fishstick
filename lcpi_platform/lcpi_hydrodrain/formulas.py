...
def _calculer_intensite_talbot(tc_min, params, verbose=False):
    ...
    if verbose:
        ...
        print("      Formule: i = a / (b + tc)")
        print(f"      AN     : i = {a} / ({b} + {tc_min:.2f})")
    if (b + tc_min) == 0:
        intensite = float("inf")
    else:
        intensite = a / (b + tc_min)
    if verbose:
        print(f"      Résultat: i = {intensite:.2f} mm/h")
    return intensite
...
def _calculer_intensite_kiefer_chu(tc_min, params, verbose=False):
    ...
    if verbose:
        ...
        print("      Formule: i = a / (tc + b)^c")
        print(f"      AN     : i = {a} / ({tc_min:.2f} + {b})^{c}")
    if (tc_min + b) <= 0:
        intensite = float("inf")
    else:
        intensite = a / ((tc_min + b) ** c)
    if verbose:
        print(f"      Résultat: i = {intensite:.2f} mm/h")
    return intensite
...
def dimensionner_section(troncon, q_max_m3s: float, verbose=False) -> dict:
    ...
    if type_section == "circulaire":
        ...
        return dimensionner_trapezoidal(
            q_max_m3s, pente, ks, troncon.largeur_fond_m, troncon.fruit_z, verbose
        )
    elif type_section == "rectangulaire":
        return dimensionner_rectangulaire(
            q_max_m3s, pente, ks, troncon.largeur_fond_m, verbose
        )
    else:
...
def dimensionner_circulaire(q_max_m3s, pente, ks, verbose=False):
    ...
    for diametre in DIAMETRES_COMMERCIAUX_m:
        ...
        q_capacite = (
            ks * section_pleine * (rayon_hydraulique_plein ** (2 / 3)) * (pente**0.5)
        )
        if verbose:
            print(f"         3. Q_capacité calculé = {q_capacite:.3f} m³/s")
        if q_capacite >= q_max_m3s:
            if verbose:
                print_colored("            -> Diamètre retenu.", "green")
            vitesse = q_max_m3s / section_pleine if section_pleine > 0 else 0
            return {
                "diametre_mm": int(diametre * 1000),
                "hauteur_m": 0,
                "largeur_m": 0,
                "vitesse_ms": vitesse,
                "section_m2": section_pleine,
                "q_capacite_m3s": q_capacite,
            }
    raise ValueError(f"Débit trop élevé ({q_max_m3s:.3f} m³/s)")


def dimensionner_trapezoidal(
    q_max_m3s, pente, ks, largeur_fond, fruit_z, verbose=False
):
    ...
    for hauteur in HAUTEURS_CANAL_m:
        ...
        rayon_hydraulique = (
            section_mouillee / perimetre_mouille if perimetre_mouille > 0 else 0
        )
        if verbose:
            print(f"         1. Section (S) = {section_mouillee:.4f} m²")
            print(f"         2. Rayon Hyd. (Rh) = {rayon_hydraulique:.3f} m")
        q_capacite = (
            ks * section_mouillee * (rayon_hydraulique ** (2 / 3)) * (pente**0.5)
        )
        if verbose:
            print(f"         3. Q_capacité calculé = {q_capacite:.3f} m³/s")
        if q_capacite >= q_max_m3s:
            if verbose:
                print_colored("            -> Hauteur retenue.", "green")
            vitesse = q_max_m3s / section_mouillee if section_mouillee > 0 else 0
            return {
                "diametre_mm": 0,
                "hauteur_m": hauteur,
                "largeur_m": largeur_fond,
                "vitesse_ms": vitesse,
                "section_m2": section_mouillee,
                "q_capacite_m3s": q_capacite,
            }
    raise ValueError(f"Débit trop élevé ({q_max_m3s:.3f} m³/s)")
