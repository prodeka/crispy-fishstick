...
(
    ...
    CAQUOT_D,
    CAQUOT_F,
) = 1.40, 0.05, 0.19, 0.84, -0.41, 0.507, -0.287
...
def run_calcul_caquot(troncon, params_pluie: dict, tc_formule_name: str, verbose=False):
    ...
    if not troncon.longueur_cumulee:
        troncon.longueur_cumulee = (
            troncon.long_parcours_surface_m + troncon.long_troncon_m
        )
        troncon.pentes_parcours = [
            (troncon.long_parcours_surface_m, troncon.pente_parcours_surface),
            (troncon.long_troncon_m, troncon.pente_troncon),
        ]

    # Calcul de la pente équivalente (I_eq) du bassin versant
    sum_lj_sqrt_ij = sum(
        longueur_segment / math.sqrt(p)
        for longueur_segment, p in troncon.pentes_parcours
        if p > 0
    )
    pente_eq = (
        (troncon.longueur_cumulee / sum_lj_sqrt_ij) ** 2 if sum_lj_sqrt_ij > 0 else 0.01
    )

    # Calcul de l'allongement (M) du bassin versant
    allongement_M = (
        troncon.longueur_cumulee / math.sqrt(troncon.surface_cumulee * 10000)
        if troncon.surface_cumulee > 0
        else 2.0
    )
    if allongement_M < 0.8:
        allongement_M = 0.8  # L'allongement est plafonné à 0.8
    ...
    troncon.tc_final_min = max(
        troncon.calculer_tc_surface(tc_formule_name), troncon.tc_amont_max
    )

    # Dimensionnement final de la section
    try:
        troncon.resultat_dimensionnement = dimensionner_section(
            troncon, troncon.q_max_m3s, False
        )
