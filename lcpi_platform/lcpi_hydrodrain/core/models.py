# core/models.py
from ..formulas import tc_kirpich, tc_californienne
...
class Troncon:
    ...
    def __init__(self, **kwargs):
        ...
        self.type_section = kwargs.get("type_section", "circulaire")
        ...
        self.amont_ids = (
            [s.strip() for s in str(kwargs.get("troncon_amont", "NONE")).split(";")]
            if str(kwargs.get("troncon_amont", "NONE")) != "NONE"
            else ["NONE"]
        )
    ...
    def calculer_tc_surface(self, methode="kirpich"):
        if methode == "kirpich":
            ...
            return tc_californienne(
                self.long_parcours_surface_m, self.pente_parcours_surface
            )
...
class Reseau:
    ...
    def __init__(self, df_input):
        self.troncons = {
            str(row["id_troncon"]): Troncon(**row) for _, row in df_input.iterrows()
        }
    ...
    def trier_topologiquement(self):
        ...
        while len(sorted_list) < len(self.troncons):
            ...
            for troncon_id, troncon in self.troncons.items():
                if troncon_id in processed_ids:
                    continue
                if troncon.amont_ids == ["NONE"] or all(
                    am_id in processed_ids for am_id in troncon.amont_ids
                ):
            ...
            if not found_new:
                ...
                raise Exception(
                    f"Erreur de topologie. Boucle ou dépendance manquante. Tronçons non traités: {unprocessed}"
                )
