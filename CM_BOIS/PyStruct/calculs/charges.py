...
def charger_psi_coeffs():
    ...
    chemin_db = os.path.join(chemin_script, '..', 'db', 'psi_coeffs.csv')
    try:
        return pd.read_csv(chemin_db)
    except FileNotFoundError:
        return None

def calculer_sollicitations_completes(longueur, charges, materiau, categorie_usage, verbose=True):
    """
    ...
    G, Q, W, S = charges.get('G', 0), charges.get('Q', 0), charges.get('W', 0), charges.get('S', 0)
    ...
    if materiau == 'acier':
        ...
        combinaisons_elu = [
            ...
            {'desc': '1.35G + 1.5Q + 1.35*0.75S', 'p_Ed': gamma_G * G + gamma_Q * Q + gamma_G * 0.75 * S},
            {'desc': '1.35G + 1.5Q + 1.35*0.75W', 'p_Ed': gamma_G * G + gamma_Q * Q + gamma_G * 0.75 * W},
        ]
        p_ser_max = G + Q + W + S
        ...
        if verbose:
            print(f"Coefficients pour categorie '{categorie_usage}': psi0={psi0}, psi2={psi2}")
        ...
        combinaisons_elu.append({'desc': 'ELU: Q en base', 'p_Ed': p_Ed1, 'formule': formule1, 'remplacement': remplacement1})
        ...
        combinaisons_elu.append({'desc': 'ELU: W en base', 'p_Ed': p_Ed2, 'formule': formule2, 'remplacement': remplacement2})
        ...
        combinaisons_elu.append({'desc': 'ELU: S en base', 'p_Ed': p_Ed3, 'formule': formule3, 'remplacement': remplacement3})
    ...
    for combo in combinaisons_elu:
        ...
        if verbose and 'formule' in combo:
            ...
            print(f"    -> Sollicitations: M_Ed = {m_ed:.2f} kNm, V_Ed = {v_ed:.2f} kNm")
        elif verbose:
            print(f"  - Combinaison: {combo['desc']:<25} -> p_Ed = {p_Ed:.2f} kN/m -> M_Ed = {m_ed:.2f} kNm")
