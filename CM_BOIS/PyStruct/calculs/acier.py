...
def charger_profils_ipe():
    """
    Lit le fichier profils_ipe.csv et retourne les données sous forme de DataFrame.
    """
    chemin_script = os.path.dirname(__file__)
    chemin_db = os.path.join(chemin_script, '..', 'db', 'profils_ipe.csv')
    try:
        df_ipe = pd.read_csv(chemin_db)
        return df_ipe
    except FileNotFoundError:
        return None

# --- FONCTION 2 : VÉRIFICATION COMPLÈTE ---
def trouver_profil_acier(M_Ed_kNm, V_Ed_kN, longueur, p_ser_kN_m, 
                         nuance='S235', fy_MPa=235, E_MPa=210000, verbose=True):
    """
    Trouve le premier profilé IPE qui vérifie la flexion, le cisaillement et la flèche.
    """
    if verbose:
        print(f"\n--- Recherche du profile acier pour M_Ed = {M_Ed_kNm:.2f} kN.m, V_Ed = {V_Ed_kN:.2f} kN et L = {longueur} m ---")

    df_profils = charger_profils_ipe()
    if df_profils is None:
        return "Erreur: Impossible de charger les donnees des profiles."
    ...
    if verbose:
        print(f"Materiau: {nuance}, Limite d'elasticite (fy): {fy} MPa, Module d'Young (E): {E} MPa")
    ...
    for index, profil in df_profils.iterrows():
        ...
        fleche_calculee_mm = (5 * charge_ser_N_mm * (longueur_mm**4)) / (384 * E * I_mm4) if (E > 0 and I_mm4 > 0) else float('inf')
        ...
        if verbose:
            ...
            print(f"     Condition: {M_pl_Rd_Nmm / 1e6:.2f} kNm >= {M_Ed_kNm:.2f} kNm ? -> {'OK' if resistance_ok else 'INSUFFISANTE'}")

            print("  2. Verification de l'effort tranchant (ELU):")
            print(f"     Condition: {V_pl_Rd_N / 1000:.2f} kN >= {V_Ed_kN:.2f} kN ? -> {'OK' if cisaillement_ok else 'INSUFFISANTE'}")

            print("  3. Verification de la fleche (ELS):")
            print(f"     Condition: {fleche_calculee_mm:.2f} mm <= {fleche_admissible_mm:.2f} mm ? -> {'OK' if fleche_ok else 'INACCEPTABLE'}")
