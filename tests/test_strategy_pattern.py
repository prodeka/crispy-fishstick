#!/usr/bin/env python3
"""
Tests pour l'architecture Strategy Pattern des solveurs hydrauliques.

Ce module teste l'implémentation du Strategy Pattern pour les solveurs
hydrauliques (LCPI Hardy-Cross et EPANET) et la factory associée.
"""
...
class TestStrategyPattern:
    """Tests pour l'architecture Strategy Pattern."""
    ...
    def test_epanet_solver_implementation(self):
        """Test l'implémentation du solveur EPANET."""
        try:
            solver = EpanetSolver()
            ...
            if "EPANET n'est pas disponible" in str(e):
                pytest.skip("EPANET non disponible pour ce test")
            else:
                raise
    ...
    def test_solver_factory_get_solver(self):
        ...
        # Test du solveur EPANET (si disponible)
        try:
            epanet_solver = SolverFactory.get_solver('epanet')
            assert isinstance(epanet_solver, EpanetSolver)
        except RuntimeError as e:
            if "EPANET n'est pas disponible" in str(e):
                pytest.skip("EPANET non disponible pour ce test")
            else:
    ...
    def test_solver_factory_capabilities(self):
        ...
        # Test des capacités du solveur EPANET (si disponible)
        try:
            epanet_capabilities = SolverFactory.get_solver_capabilities('epanet')
            assert 'info' in epanet_capabilities
            assert 'supported_formulas' in epanet_capabilities
            assert 'default_parameters' in epanet_capabilities
        except RuntimeError as e:
            if "EPANET n'est pas disponible" in str(e):
                pytest.skip("EPANET non disponible pour ce test")
            else:
    ...
    def test_network_validation(self):
        ...
        # Test avec le solveur EPANET (si disponible)
        try:
            epanet_solver = EpanetSolver()
            epanet_validation = epanet_solver.validate_network(valid_network)
            assert epanet_validation['valid'] is True
            assert len(epanet_validation['errors']) == 0
        except RuntimeError as e:
            if "EPANET n'est pas disponible" in str(e):
                pytest.skip("EPANET non disponible pour ce test")
            else:
    ...
    def test_network_validation_invalid(self):
        ...
        try:
            epanet_solver = EpanetSolver()
            epanet_validation = epanet_solver.validate_network(invalid_network)
            assert epanet_validation['valid'] is False
            assert len(epanet_validation['errors']) > 0
            assert any("reservoir" in error.lower() for error in epanet_validation['errors'])
        except RuntimeError as e:
            if "EPANET n'est pas disponible" in str(e):
                pytest.skip("EPANET non disponible pour ce test")
            else:
    ...
    def test_solver_factory_validation(self):
        ...
        # Test de validation pour EPANET (si disponible)
        try:
            epanet_validation = SolverFactory.validate_solver_choice('epanet', test_network)
            assert epanet_validation['solver_name'] == 'epanet'
            assert epanet_validation['compatible'] is True
            assert 'validation' in epanet_validation
            assert 'capabilities' in epanet_validation
        except RuntimeError as e:
            if "EPANET n'est pas disponible" in str(e):
                pytest.skip("EPANET non disponible pour ce test")
            else:
    ...
    def test_solver_parameters(self):
        ...
        # Test des paramètres EPANET (si disponible)
        try:
            epanet_solver = EpanetSolver()
            epanet_params = epanet_solver.get_solver_parameters()
            assert 'tolerance' in epanet_params
            assert 'max_iterations' in epanet_params
            assert 'formula' in epanet_params
            assert 'duration_hours' in epanet_params
            assert 'timestep_minutes' in epanet_params
            assert 'quality_type' in epanet_params
        except RuntimeError as e:
            if "EPANET n'est pas disponible" in str(e):
                pytest.skip("EPANET non disponible pour ce test")
            else:
    ...
    def test_solver_formulas(self):
        ...
        # Test des formules EPANET (si disponible)
        try:
            epanet_solver = EpanetSolver()
            epanet_formulas = epanet_solver.get_supported_formulas()
            assert 'hazen_williams' in epanet_formulas
            assert 'darcy_weisbach' in epanet_formulas
            assert 'chezy_manning' in epanet_formulas
        except RuntimeError as e:
            if "EPANET n'est pas disponible" in str(e):
                pytest.skip("EPANET non disponible pour ce test")
            else:
...
class TestStrategyPatternIntegration:
    """Tests d'intégration pour l'architecture Strategy Pattern."""
    ...
    def test_solver_interchangeability(self):
        ...
        # Test avec EPANET (si disponible)
        try:
            epanet_solver = SolverFactory.get_solver('epanet')
            epanet_results = epanet_solver.run_simulation(test_network)
            
            # Vérifier la structure des résultats
            assert 'status' in epanet_results
            assert 'solver' in epanet_results
            assert 'convergence' in epanet_results
            assert epanet_results['solver'] == 'epanet'
